...
<3> This is a nice way to include the app config into the logs.
...
<2> The configuration can be mocked from any test method or from a method annotated with one of JUnit's https://junit.org/junit5/docs/current/user-guide/#writing-tests-definitions[lifecycle annotations] such as `@BeforeEach`.
...
They can be mocked too, even though it's a little trickier:
...
----
<1> Be careful about the import. This class comes from MicroProfile Config.
<2> This annotation can be omitted.
<3> We need to make the interface implementation proxyable, so that it can be mocked with `@InjectMock`.
