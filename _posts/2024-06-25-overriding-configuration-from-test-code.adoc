...
----
<1> The configuration is overridden from the test when the bean constructor is invoked.
...
@TestConfigProperty(key = "awesome-feature-enabled", value = "true") // <2>
...
System properties can be used to override the configuration from the test code, but that only works once with `@ApplicationScoped` or `@Singleton` beans, before they've been initialized.
After their initialization, any changes made to system properties will have no effect on these beans.
...
class FeaturesResourceTest {
    ...
    @Order(1) // <2>
    void firstTest() {
        System.setProperty("awesome-feature-enabled", "true"); // <3>
        RestAssured.given()
                .when().get("/features/awesome")
                .then().body(CoreMatchers.is("true"));
    }
    ...
    void lastTest() {
        ...
        RestAssured.given()
                .when().get("/features/awesome")
                .then().body(CoreMatchers.is("true")); // <5>
    }
}
----
...
<3> This has no effect when the test is run in native mode.
<4> This test is always run last.
<5> This test depends on a CDI bean with a default `@Singleton` scope which was already initialized by the previous test.
As a consequence, the outcome of this test cannot be changed from the system property.

== Off-topic: the `application.properties` file

You may wonder why I didn't mention the `application.properties` file sooner in this post.
It can indeed be used to change the configuration of a Quarkus app while testing it.

[source, properties, title=application.properties]
----
awesome-feature-enabled=false
%test.awesome-feature-enabled=true
----

There is however a big difference between that file and all the approaches listed above:

* the `application.properties` file is a static resource which cannot be changed while running the tests
* all approaches from this post are dynamic and used from the test code
