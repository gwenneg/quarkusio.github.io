...
Injecting the configuration in your CDI beans constructors might be the right approach for you.
...
public class FeaturesServiceTest {
    ...
    @Test
    void testFalse() {
        FeaturesService featuresService = new FeaturesService(false); // <1>
        Assertions.assertFalse(featuresService.isAwesomeFeatureEnabled());
    }
...
----
<1> The configuration is overridden from the test when the CDI bean constructor is invoked.




This approach suffers from an obvious drawback: creating an instance from the test code will break CDI.
